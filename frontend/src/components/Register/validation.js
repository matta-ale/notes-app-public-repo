export function validation(userData) {
    ...
    if (!regexPassword.test(userData.password)) errors.password = "Password must have at least one special character, a number, an uppercase and lowercase letter"
    if (userData.password.length < 6 || userData.password.length > 14) errors.password = "Password must have between 6 and 14 characters"

    if (!regexPassword.test(userData.repeatPassword)) errors.repeatPassword = "Password must have at least one special character, a number, an uppercase and lowercase letter"
    if (userData.password.length < 6 || userData.password.length > 14) errors.password = "Password must have between 6 and 14 characters"
